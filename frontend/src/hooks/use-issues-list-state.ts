...
export function useIssuesListState({
  ...
  // Filtered issues (computed first so KPI counts can derive from them)
  ...
  const kpiCounts = useMemo(() => {
    return {
      ...
      open: filteredIssues.filter((i) => !["Resolved", "Closed"].includes(i.status)).length,
      critical: filteredIssues.filter(
        (i) => i.severity === "Critical" && !["Resolved", "Closed"].includes(i.status)
      ).length,
      overdue: filteredIssues.filter(isIssueOverdue).length,
